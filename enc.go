...
import (
	...
	"github.com/wallyqs/old-nats-encoders.go/builtin"
)
...
// encoder.
func NewEncodedConn(c *nats.Conn, encType string) (*EncodedConn, error) {
	if c == nil {
...
func (c *EncodedConn) Subscribe(subject string, cb Handler) (*nats.Subscription, error) {
...
func (c *EncodedConn) QueueSubscribe(subject, queue string, cb Handler) (*nats.Subscription, error) {
	return c.subscribe(subject, queue, cb)
}

// Internal implementation that all public functions will use.
func (c *EncodedConn) subscribe(subject, queue string, cb Handler) (*nats.Subscription, error) {
	if cb == nil {
	...
	natsCB := func(m *nats.Msg) {
		...
		if wantsRaw {
			...
			if err := c.Enc.Decode(m.Subject, m.Data, oPtr.Interface()); err != nil {
				// FIXME: Add support for async callback of errors again?
